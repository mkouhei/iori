...
class Repo(object):
    ...
    def git_init(self):
        ...
        if not self.git_repo:
            ...
            gitignore_path = os.path.join(self.dirpath, '.gitignore')
            self.write_file(gitignore_path)
            self.git_add_commit(gitignore_path)
            ...
            # generate README
            readme_path = os.path.join(self.dirpath, 'README')
            msg = ('This repository is generate automatically by iori.\n'
                   '"master" branch is mapping of relation lxc host node '
                   'and git ripository\n'
                   'branches. Other branches exception of master and '
                   'template are created by\n'
    ...
    def git_add_commit(self, filename):
        ...
        if self.contname:
            msg = '%s: %s %s' % (self.contname, action, filename)
        else:
